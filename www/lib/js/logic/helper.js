...
var apphelper = {
    ...
    */
    convertInAllFilesizes: function(obj){
        // TODO Am besten Translator-Modul für die Formatierung der Text-Werte nutzen 
        // (e.g. Komma und Punkt-Nutzung), siehe translator.js
        var result = {};
    ...
    */
    convertPath: function (obj) {
        ...
        // Sonderbehandlung für Dateien
        if (!obj.isDir) {

            // Testen, ob die Datei eine Endung hat und nicht mit Endung besteht
    ...
    sortByName: function (first, second) {
        //Ordner werden oben aufgeführt, da sie keine Größe haben
        if (first.isDir && !second.isDir) {
            return -1;
        } else if (!first.isDir && second.isDir) {
            return 1;
        } else if (first.isDir && second.isDir) {
            //Benutze Ordnernamen zum sortieren
            ...
            //Wenn es kein Ordner ist, sortiere einfach alphabetisch    
        } else {
            if (first.fileName.toLowerCase() == second.fileName.toLowerCase()) {
                return 0;
            }
    ...
    sortBySizeDesc: function (first, second) {
        ...
        if (first.isDir && !second.isDir) {
            ...
            //Wenn es kein Ordner ist, sortiere einfach nach größe    
        } else {
